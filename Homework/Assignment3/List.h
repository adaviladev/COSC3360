...
public:
	...
	void addNode( Node< T > *cur , T prefix , T injection ) {    // used to add a node after @prefix
		if( cur != NULL ) {
			Node< T > *temp;
			if( prefix ==
			    "" ) {                                            // used to match the </injection> case of the insertword command
				...
				if( cur->next !=
				    NULL ) {                            // used to check if the current node is the last node // Should have just compared it with last...
					temp->next = cur->next;
					temp->prev = cur;
					cur->next->prev = temp;
					cur->next = temp;
	...
	void display(
			Node< T > *cur ) {                    // prints out node's word to console then calls itself with the next node
		if( cur != NULL ) {
//			cout << cur->data << endl;
			cout << cur->data.procId << "\t";
			cout << cur->data.addr.hexAddress << "\t";
			cout << cur->data.addr.binAddress << endl;
			display( cur->next );
		} else {
